...
<body>
    <div class="header">  
        ...
        <div class="user-section">
            ...
            <div>
                <a herf='profile.php'><i class="fa-solid fa-user"></i></a>
            </div>
        </div>
    </div>
    <div class="middle" style="background-image: url('<?php echo "images/" . substr(basename($_SERVER['PHP_SELF']), 0, strpos(basename($_SERVER['PHP_SELF']), ".")) . "_1.jpg"; ?>');">
    ...
    <div class="products">
        ...
        $query = "SELECT * FROM seq_product WHERE category='$a' ORDER BY `$email` DESC";
        $rlt = $conn->query($query);
        while ($r_w = mysqli_fetch_assoc($rlt)) {
                $product = mysqli_fetch_assoc($conn->query("SELECT * FROM products where p_id={$r_w['p_id']}"));
    ...
    <div class="end" style="background-image: url('<?php echo "images/" . substr(basename($_SERVER['PHP_SELF']), 0, strpos(basename($_SERVER['PHP_SELF']), ".")) . "_2.jpg"; ?>');">
